...
/system identity set name="ES_USER3_GF_MT-hEX-S"
...
#######################################
 ...
 /interface ethernet
   ...
   set comment="UNMANAGED SWITCH HOME-OFFICE"					[find name=ether2]  
   set comment="not used"										[find name=ether3]
   set comment="LIVING ROOM"									[find name=ether4]
   set comment="ACCESS POINT cAP ac, PoE-Out"					[find name=ether5]   
...
#######################################
  ...
  /interface bridge port
    ...
    add bridge=bridge-VLANs interface=ether2 pvid=130 ingress-filtering=yes\
	    frame-types=admit-only-untagged-and-priority-tagged comment="UNMANAGED SWITCH HOME-OFFICE"
    add bridge=bridge-VLANs interface=ether4 pvid=130 ingress-filtering=yes\
	    frame-types=admit-only-untagged-and-priority-tagged comment="LIVING ROOM"
...
#######################################
  ...
  /interface bridge port
	...
	add bridge=bridge-VLANs interface=ether5 	comment="TAGGED PORT, VLAN-TRUNK TO ACCESS POINT cAP ac, PoE-Out"
	
  # add VLANs to be allowed on the tagged ports Only these will be trunked through the connected wire
  /interface bridge vlan
    add bridge=bridge-VLANs vlan-ids=1   comment="BLACKHOLE"  		 			tagged=sfp1,ether5
	add bridge=bridge-VLANs vlan-ids=99  comment="BASE (MGMT) VLAN"  			tagged=sfp1,ether5,bridge-VLANs
	add bridge=bridge-VLANs vlan-ids=111 comment="MAIN VLAN OF USER1 FOR WLAN" 	tagged=sfp1,ether5
	add bridge=bridge-VLANs vlan-ids=121 comment="MAIN VLAN OF USER2 FOR WLAN" 	tagged=sfp1,ether5
	add bridge=bridge-VLANs vlan-ids=130 comment="MAIN VLAN OF USER3 FOR LAN" 	tagged=sfp1,ether5	
	add bridge=bridge-VLANs vlan-ids=131 comment="MAIN VLAN OF USER3 FOR WLAN" 	tagged=sfp1,ether5      
	add bridge=bridge-VLANs vlan-ids=141 comment="MAIN VLAN OF USER4 FOR WLAN" 	tagged=sfp1,ether5      
	add bridge=bridge-VLANs vlan-ids=151 comment="MAIN VLAN OF USER5 FOR WLAN" 	tagged=sfp1,ether5
	add bridge=bridge-VLANs vlan-ids=161 comment="MAIN VLAN OF USER6 FOR WLAN" 	tagged=sfp1,ether5      
	add bridge=bridge-VLANs vlan-ids=171 comment="MAIN VLAN OF USER7 FOR WLAN" 	tagged=sfp1,ether5      
	add bridge=bridge-VLANs vlan-ids=181 comment="MAIN VLAN OF USER8 FOR WLAN" 	tagged=sfp1,ether5      
	add bridge=bridge-VLANs vlan-ids=201 comment="GUEST WLAN"  					tagged=sfp1,ether5
...
#######################################
  ...
  /ip address  
	add address=10.99.99.130/24 	interface=VLAN_099	 comment="BASE (MGMT) VLAN"
