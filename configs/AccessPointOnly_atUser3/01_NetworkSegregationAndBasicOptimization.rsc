...
# RouterOS:		6.48.3
# Device:		https://mikrotik.com/product/cap_ac
# Date:			June 18, 2021
...
/system identity set name="AP_USER3_GF_MT-cAP-ac"
...
#######################################
 ...
 /interface ethernet
   set comment="PoE-In, to ES_USER3"								[find name=ether1]
   set comment="PoE-Out, MANAGEMENT TERMINAL, LOCAL BACKUP PORT" 	[find name=ether2]

  /interface wireless   
   set comment="2.4 GHz PHYSICAL WLAN INTERFACE"					[find name=wlan1]
   set comment="5 GHz PHYSICAL WLAN INTERFACE"						[find name=wlan2]  
   set name="wlan2GHz"												[find name=wlan1]
   set name="wlan5GHz"												[find name=wlan2]
...
#######################################
  ...
  /interface bridge port
    add bridge=bridge-VLANs interface=ether2 pvid=99 ingress-filtering=yes\
...
#######################################
  ...
  /interface bridge port
	add bridge=bridge-VLANs interface=ether1 	comment="TAGGED PORT, VLAN-TRUNK TO EDGE SWITCH hEX S, PoE-In"
...
#######################################
  ...
  /ip address  
	add address=10.99.99.131/24 	interface=VLAN_099	 comment="BASE (MGMT) VLAN"
