...
# RouterOS:		6.48.3
# Device:		https://mikrotik.com/product/RB960PGS
# Date:			June 18, 2021
...
/system identity set name="ES_PoEtoAPs_1B_MT-hEX-PoE"
...
#######################################
 ...
 /interface ethernet
   set comment="TAGGED PORT, VLAN-TRUNK TO CORE-SWITCH-SFP" 	[find name=sfp1]
   set comment="MANAGEMENT TERMINAL, LOCAL BACKUP PORT"			[find name=ether1]
   set comment="ACCESS POINT hAP ac3, 1B, PoE-Out"				[find name=ether2]  
   set comment="ACCESS POINT hAP ac3, GF, PoE-Out"				[find name=ether3]
   set comment="ACCESS POINT hAP ac3, 1F, PoE-Out"				[find name=ether4]
   set comment="not used"										[find name=ether5]   
...
#######################################
  ...
  /interface bridge port
	add bridge=bridge-VLANs interface=sfp1 		comment="TAGGED PORT, VLAN-TRUNK TO CORE-SWITCH-SFP"
	add bridge=bridge-VLANs interface=ether2 	comment="TAGGED PORT, VLAN-TRUNK TO ACCESS POINT hAP ac3, 1B, PoE-Out"
	add bridge=bridge-VLANs interface=ether3	comment="TAGGED PORT, VLAN-TRUNK TO ACCESS POINT hAP ac3, GF, PoE-Out"
	add bridge=bridge-VLANs interface=ether4	comment="TAGGED PORT, VLAN-TRUNK TO ACCESS POINT hAP ac3, 1F, PoE-Out"
	
  # add VLANs to be allowed on the tagged ports Only these will be trunked through the connected wire
  /interface bridge vlan
    add bridge=bridge-VLANs vlan-ids=1   comment="BLACKHOLE"  		 			tagged=sfp1,ether2,ether3,ether4
	add bridge=bridge-VLANs vlan-ids=99  comment="BASE (MGMT) VLAN"  			tagged=sfp1,ether2,ether3,ether4,bridge-VLANs
	add bridge=bridge-VLANs vlan-ids=111 comment="MAIN VLAN OF USER1 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4
	add bridge=bridge-VLANs vlan-ids=121 comment="MAIN VLAN OF USER2 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4
	add bridge=bridge-VLANs vlan-ids=130 comment="MAIN VLAN OF USER3 FOR LAN" 	tagged=sfp1,ether3	
	add bridge=bridge-VLANs vlan-ids=131 comment="MAIN VLAN OF USER3 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4      
	add bridge=bridge-VLANs vlan-ids=141 comment="MAIN VLAN OF USER4 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4      
	add bridge=bridge-VLANs vlan-ids=151 comment="MAIN VLAN OF USER5 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4
	add bridge=bridge-VLANs vlan-ids=160 comment="MAIN VLAN OF USER6 FOR LAN" 	tagged=sfp1,ether4
	add bridge=bridge-VLANs vlan-ids=161 comment="MAIN VLAN OF USER6 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4      
	add bridge=bridge-VLANs vlan-ids=171 comment="MAIN VLAN OF USER7 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4      
	add bridge=bridge-VLANs vlan-ids=181 comment="MAIN VLAN OF USER8 FOR WLAN" 	tagged=sfp1,ether2,ether3,ether4      
	add bridge=bridge-VLANs vlan-ids=201 comment="GUEST WLAN"  					tagged=sfp1,ether2,ether3,ether4
...
#######################################
  ...
  /ip address  
	add address=10.99.99.100/24 	interface=VLAN_099	 comment="BASE (MGMT) VLAN"
